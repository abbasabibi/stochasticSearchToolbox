...
terminationPolicy.setTheta(rand(1,dataManager.getNumDimensions('statesSquared')) -0.5);
...
mixtureModel  = Distributions.MixtureModel.MixtureModelWithTermination.createPolicy(dataManager, gatingDist, optionInitializer, 'actions', 'states', squaredFeatures.outputName,...
...
environment = Learner.ExpectationMaximization.test.HMMTestEnv(dataManager);
...
newData.setDataEntry('states', randn(numSamples,dimState), :,1 ); %[episode, step]
...
terminationMMLearner = Learner.SupervisedLearner.TerminationMMLearner(dataManager, mixtureModel, optionLearner, 'responsibilities');
