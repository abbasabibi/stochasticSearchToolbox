...
settings.setProperty('doKMeansInit', doKMeansInit);
settings.setProperty('keepOptionsShape', keepOptionsShape);
settings.setProperty('learnInputShapeKMeans', learnInputShape);
settings.setProperty('learnOutputShapeKMeans', learnOutputShape);
settings.setProperty('numInitStatesKMeans', 1e2);
...
dataManager.addDataEntryForDepth(depth, 'parametersOld', dataManager.getNumDimensions('actions')*dataManager.getNumDimensions('states') );
...
controller          = TrajectoryGenerators.TrajectoryTracker.LinearFeedbackController(dataManager, 1+2);
...
gaussianDist  = Distributions.Gaussian.GaussianLinearInFeatures(dataManager, 'parameters', '', 'ActionPolicy');
...
mixtureModel  = Distributions.MixtureModel.MixtureModelExploration.createParameterPolicy(...
    dataManager, gatingDist, optionInitializer, 'parameters', 'states', terminationPolicy.inputVariables{1},...
    terminationPolicyInitializer,'options','optionsOld');


sampler.setActionPolicy(controller);
sampler.stepSampler.addSamplerFunctionToPool('Policy', 'sampleFromDistribution', mixtureModel, -1);
...
EMLearner           = Learner.ExpectationMaximization.EMExploration (dataManager, mixtureModel, mixtureModelLearner);
...
plotter = Evaluator.PendulumPlotterExploration(repsLearner, dataManager, contextFeatures.outputName, gatingDist.inputVariables{1}, sampler);
