...
settings.setProperty('maxSamples', 300);
...
endStageSampler     = Sampler.test.TransitionContextSampler(dataManager, 6, contextSampler); %Change stuff in here
...
dataManager.setRange('contexts', [-5 environment.field.ballInitHeight -0.3 -1 0 -20], ...
    [5 environment.field.ballInitHeight 0.3 -1 0 20]);

dataManager.addDataAliasForDepth(2,'contextsForFeatures', 'contexts', [1,3,6]);
dataManager.addDataAliasForDepth(2,'nextContextsForFeatures', 'nextContexts', [1,3,6]);
dataManager.finalizeDataManager();



squaredFeatures     = FeatureGenerators.SquaredFeatures(dataManager, 'contextsForFeatures', ':', true);
nextSquaredFeatures = FeatureGenerators.SquaredFeatures(dataManager, 'nextContextsForFeatures', ':', true);
...
mixtureModel        = Distributions.MixtureModel.MixtureModel.createParameterPolicy(...
    dataManager, gatingDist, optionInitializer, 'parameters', 'contextsForFeaturesSquared', 'options');
...
repsLearner     = Learner.SteadyStateRL.HiREPSIter(dataManager, mixtureModelLearner, 'returns', 'returnWeightings', ...
    'responsibilities', 'contextsForFeaturesSquared', 'contextsForFeaturesSquaredSampleModel');
