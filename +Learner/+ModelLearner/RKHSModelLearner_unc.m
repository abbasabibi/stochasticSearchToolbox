classdef RKHSModelLearner_unc < Learner.ModelLearner.RKHSModelLearner
    ...
    methods
        function obj = RKHSModelLearner_unc(dataManager, stateIndices,sfeatureExtractor,nsfeatureExtractor, safeatureExtractor)
        ...
        function [features] = getFeaturesInternal(obj, numFeatures, inputMatrix)
            ...
            cond_embed = (K_sa + obj.regularizer*eye(size(K_sa)))\K_sa_in; %one column for every (S,A) predict
        ...
        function objective = optimizationObjective(obj, data, params)
            ...
            for i = 1:2
                ...
                sakf = @(sa1,sa2) obj.safeatureExtractor.kernel.getGramMatrix(sa1,sa2);
                ...
                wts = (Kt_in + obj.regularizer * eye(size(Kt_in))) \ Ktv_in;
