...
if exist('x_0','var')
    if exist('sigma','var')
        [x_0, sigma, alpha] = Optimizer.EvolutionaryStrategies.validate_sizes(mu, n, limits, x_0, sigma);
    else
        [x_0, sigma, alpha] = Optimizer.EvolutionaryStrategies.validate_sizes(mu, n, limits, x_0);
    end
else
    [x_0, sigma, alpha] = Optimizer.EvolutionaryStrategies.validate_sizes(mu, n, limits);
end
...
while ((j < gen) && (min(eps) > e))
    ...
    %% Recombine:
    [xr,sigmar,alphar] = Optimizer.EvolutionaryStrategies.recombination(rec_obj,rec_str,n,mu,lambda,min_x{j},sigma,alpha);
    off{j+1}           = xr;            % offspring population
    
    %% Mutation:
    [xm,sigmam,alpham] = Optimizer.EvolutionaryStrategies.mutation(n,lambda,xr,sigmar,alphar,limits);
    ...
    %% Selection:
    [min_x{j+1}, sigma, alpha] = Optimizer.EvolutionaryStrategies.selection(sel, mu, lambda, epse, eps, xm, sigmam, min_x{j}, sigma, alpham, alpha);
