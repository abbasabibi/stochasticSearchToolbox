function h=plotgauss2d(mu, Sigma, color, width)
...
h = plotcov2(mu, Sigma, color, width);
return;
...
function h = plotcov2(mu, Sigma, color, width, varargin)
...
[p, ...
 n, ...
 plot_opts] = Gaussianplot.process_options(varargin, 'conf', 0.9, ...
					'num-pts', 100);
h = [];
holding = ishold;
if (Sigma == zeros(2, 2))
  ...
  L = k * sqrt(diag(D));
  %h = plot([mu(1); mu(1) + L(1) * V(1, 1)], [mu(2); mu(2) + L(1) * V(2, 1)], plot_opts{:});
  hold on;
 % h = [h; plot([mu(1); mu(1) + L(2) * V(1, 2)], [mu(2); mu(2) + L(2) * V(2, 2)], plot_opts{:})];
end

h = [h; plot(z(1, :), z(2, :), plot_opts{:}, 'Color', color, 'LineWidth', width)];
hf = fill(z(1, :), z(2, :), color);
set(hf,'facealpha',.5)
%plot(mu(1),mu(2), 'x', 'MarkerSize',15, 'LineWidth', width + 1, 'Color', color);
