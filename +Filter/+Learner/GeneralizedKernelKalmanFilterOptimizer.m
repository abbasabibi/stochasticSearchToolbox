classdef GeneralizedKernelKalmanFilterOptimizer < Learner.ParameterOptimization.AbstractHyperParameterOptimizer
...
%         inputDataEntry
    ...
    methods
        function obj = GeneralizedKernelKalmanFilterOptimizer(dataManager, gkkfLearner, optimizationName)
...
%             end
        ...
        function [funcVal, gradient] = objectiveFunction(obj, params)
            ...
            switch obj.testMethod
                case 'filter'
                    ...
                    if not(isempty(obj.validityDataEntry))
                        ...
                        valid = all(valid,2);
                        valid = valid(1:size(observations,2));
                    else
            ...
            switch obj.internalObjective
                ...
                case 'nmse'
                    funcVal = funcVal + obj.normalizedSquaredError(groundtruth,mu)/length(groundtruth(:));
                    if exist('muMonitoring','var')
                        fprintf(' %f ',obj.squaredError(groundtruthMonitoring,muMonitoring)/length(groundtruthMonitoring(:)));
                    end
                case 'llh'
...
%             obj.hyperParameterObject.updateKernelReferenceSets(obj.wholeData);
    ...
    methods (Static)
        ...
        function error = normalizedSquaredError(data, estimates)
            error = (estimates - data).^2;
            error = bsxfun(@rdivide,sum(error,2),var(data,0,2));
            error = sum(error(:));
        end
