classdef KernelBayesFilterLearner < Learner.Learner & Learner.ParameterOptimization.HyperParameterObject
    ...
    methods
        function obj = KernelBayesFilterLearner(dataManager, name)
            ...
            obj.stateKernelReferenceSet = Kernels.KernelReferenceSet(dataManager, stateKernel, 'stateKRS');
            obj.stateKernelReferenceSet.inputDataEntryReferenceSet = obj.stateFeatureName;
            obj.obsKernelReferenceSet = Kernels.KernelReferenceSet(dataManager, obsKernel, 'obsKRS');
            obj.obsKernelReferenceSet.inputDataEntryReferenceSet = obj.obsFeatureName;
            obj.reducedKernelReferenceSet = Kernels.KernelReferenceSet(dataManager, stateKernel, 'reducedKRS');
            obj.stateKernelReferenceSet.inputDataEntryReferenceSet = obj.stateFeatureName;

            switch (obj.conditionalOperatorType)
                case 'std'
                    if obj.windowPrediction
                        obj.filter = Filter.WindowPredictionKernelBayesFilter(dataManager, obj.stateKernelReferenceSet, obj.obsKernelReferenceSet);
                    else
                        obj.filter = Filter.KernelBayesFilter(dataManager, obj.stateKernelReferenceSet, obj.obsKernelReferenceSet);
                    end
...
%                         obj.filter = Filter.WindowPredictionRegGeneralizedKernelKalmanFilter(dataManager, obj.stateKernelReferenceSet, obj.obsKernelReferenceSet);
                        ...
                        obj.filter = Filter.SubSpaceKernelBayesFilter(dataManager, obj.stateKernelReferenceSet, obj.obsKernelReferenceSet);
                    end
                    obj.transitionModelLearner = Filter.Learner.TransitionModelLearnerSubspaceBayes(dataManager, obj.filter, obj.stateFeatureName, obj.reducedKernelReferenceSet);
                    obj.observationModelLearner = Filter.Learner.ObservationModelLearnerSubspaceBayes(dataManager, obj.filter, obj.stateFeatureName, obj.reducedKernelReferenceSet);
                    obj.stateKRSL = Kernels.Learner.RandomKernelReferenceSetLearner(dataManager, obj.stateKernelReferenceSet);
                    obj.obsKRSL = Kernels.Learner.RandomKernelReferenceSetLearner(dataManager, obj.obsKernelReferenceSet);
                    switch (obj.referenceSetLearnerType)
                        case 'greedy'
