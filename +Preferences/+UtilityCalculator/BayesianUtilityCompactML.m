classdef BayesianUtilityCompactML < Preferences.UtilityCalculator.AbstractUtilityFunctionCalculator
    ...
    methods
        function obj =  BayesianUtilityCompactML(dataManager,featureName,varargin)
            ...
            obj.solver = Preferences.UtilityCalculator.Bayesian.BayesPreferenceSolverCompact(chol(obj.covm),obj.sampleCount,obj.shapeM,obj.use01LL);
        ...
        function [weights] = calculateUtilityFunction(obj,c,trajs,fe,prefs,iteration,importance)
            ...
            if preferenceCount==0 || size(trajs,1)<2
                ...
                fprintf('Optimizing over %i preferences',preferenceCount);
                %value differences
                ...
                params = zeros(size(trajs,1),1);
                obj.solver.cholPrior = chol(eye(numel(params)));
                [theta, logL] = obj.solver.solve(params',A,B,trajs,timportance);
                theta = theta*trajs;
