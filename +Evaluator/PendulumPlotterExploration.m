classdef PendulumPlotterExploration < Evaluator.Evaluator
    ...
    methods
        function [obj] = PendulumPlotterExploration (repsLearner, dataManager, featureNameValue, featureNameGating, sampler)
...
%                 Plotter.PlotterData.plotTrajectories(newData,'parameters',1,26);
                ...
                for i = 1 : length(optionList)
                    ...
                    parameters          = learner.functionApproximator.options{option}.getExpectation(numSamples, contextsForOption);
                    
                    policyExpectation(sampleSelection) =  parameters(:,1) + sum(parameters(:,2:3) .* contextsForOption,2);
                end
                
                policyExpectation               = min(max(policyExpectation,-50), 50);
...
%                 print(['plots/PolicyMatlabIter',num2str(trial.iterIdx)],'-dsvg')
                ...
                else if( isprop(obj.repsLearner.policyLearner, 'mixtureModelLearner') )
                        colors = obj.repsLearner.policyLearner.mixtureModelLearner.colors;
                    else
                        colors = learner.colors;
                    end
                end
