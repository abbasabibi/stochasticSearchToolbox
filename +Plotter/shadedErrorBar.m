function H=shadedErrorBar(x,y,errBar,lineProps,transparent,plotLog, withEdge, lineProps2)
...
if(~exist('withEdge','var')||isempty(withEdge))
    withEdge = true;
end
...
if nargin<8 || isempty(lineProps2)
    lineProps2={'-','color',edgeColor}; 
end

if(withEdge)
    if plotLog
        H.edge(1)=semilogy(x,lE,lineProps2{:});
        H.edge(2)=semilogy(x,uE,lineProps2{:});
    else    
        H.edge(1)= semilogy(x,lE,lineProps2{:});
        H.edge(2)= semilogy(x,uE,lineProps2{:});
    end
