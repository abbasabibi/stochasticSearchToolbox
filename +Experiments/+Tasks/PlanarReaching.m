classdef PlanarReaching < Experiments.Tasks.StepBasedTask
    ...
    methods
        function obj = PlanarReaching()
            obj = obj@Experiments.Tasks.StepBasedTask('PlanarReaching', false);
        end
        ...
        function setupViaPoint(obj,trial)
            ...
            if(isempty(trial.viaPoint))
                trial.viaPoint.times   = [40, 100]; 
                trial.viaPoint.factors = repmat([1e4, 1e4, 0, 0], length(trial.viaPoint.times), 1);
                trial.viaPoint.points{1}  = [1.0, 1.0, 0.0, 0.0];
                trial.viaPoint.points{2}  = [trial.numJoints, 0.0, 0.0, 0.0];                
                trial.viaPoint.uFactor = 0.5 * 10^0;
