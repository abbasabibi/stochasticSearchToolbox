...
baseFolder = 'C:/Users/cwirth/workspace/policysearchtoolbox/+Experiments/data/testkmeans/';
%baseFolder = '~/policysearchtoolbox/+Experiments/data/bestPreBicycle/';

Plotter.PlotterEvaluations.plotMode(1);
...
measure = {'settings.trajPrefsPerIteration','settings.initPrefs','settings.epsilonAction'};
scale =  'p = %1.0f i = %1.0f e = %1.3f';
labelGen = @(x1_,x2_,x3_) sprintf(scale, x1_, x2_,x3_);
...
for k=1:length(names)
    ...
    for l=1:length(exNames)
        ...
        if sum(ismember(fNames,{'experiment.mat'}))==1
            ...
            [dataPlot] = Plotter.PlotterEvaluations.preparePlotData(data, 'reqPrefs', returnName, measure, labelGen, avgStr, false, [], []); %last two p: trial,evals
