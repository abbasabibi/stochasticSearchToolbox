...
category = 'evalQuadLinkSmoothing';
%category = 'test';
experimentName = 'evalParameters_WinSize';

% set some variables
windowSize = {5 5 10 40;
              6 6 12 41;
              7 7 14 42;
              8 8 16 43;
              9 9 18 44;
...
evaluate = Experiments.Evaluation(...
    {'settings.windowSize','settings.windowPreprocessor_windowSize','settings.filterLearner_stateFeatureSize','settings.numTimeSteps'},windowSize,numIterations,numTrials);
...
% evaluate.setDefaultParameter('settings.numTimeSteps',20);
...
% general settings
% evaluate.setDefaultParameter('settings.windowSize', 2);
...
evaluate.setDefaultParameter('settings.observationPointsPreprocessor_observationIndices',[1:5,35]);


% window settings
windowsPreproName = 'windowsPrepro';
evaluate.setDefaultParameter('settings.windowPreprocessor_inputNames', {'endEffPositionsNoisy'});
% evaluate.setDefaultParameter('settings.windowPreprocessor_windowSize', 2);
...
evaluate.setDefaultParameter('settings.GKKS_lambdaO',exp(-11));
evaluate.setDefaultParameter('settings.GKKS_lambdaT',exp(-11));
evaluate.setDefaultParameter('GKKS_lambdaI',exp(-13.5));
...
evaluate.setDefaultParameter('settings.reducedKRS_maxSizeReferenceSet', 750);
...
evaluate.setDefaultParameter('settings.obsKRS_kernelMedianBandwidthFactor', .9);
evaluate.setDefaultParameter('settings.stateKRS_kernelMedianBandwidthFactor', .9);
