classdef GeneralizedKernelKalmanSmootherOptimizer < Learner.ParameterOptimization.AbstractHyperParameterOptimizer
...
%         inputDataEntry
    ...
    methods
        function obj = GeneralizedKernelKalmanSmootherOptimizer(dataManager, gkkfLearner, optimizationName)
        ...
        function [funcVal, gradient] = objectiveFunction(obj, params)
            ...
            switch obj.testMethod
                case 'smooth'
                    ...
                    if ~isempty(obj.validityDataEntry)
                    ...
                    [mu, var] = obj.hyperParameterObject.filter.smoothData(permute(observations,[2,3,1]),obsPoints);
                ...
                case 'experimentalObs'
                    ...
                    mu1 = obj.hyperParameterObject.filter.GL * stateEmbeddings;
                    groundtruth1 = obj.hyperParameterObject.filter.Kro * obsEmbeddings;
                    
                    mu2 = obj.oneStepPredictions(states);
                    groundtruth2 = obj.testData.getDataEntry(obj.groundtruthName);
                    groundtruth2 = groundtruth2(find(valid)+1,:);
            ...
            switch obj.internalObjective
                ...
                case 'rmse'
                    funcVal = funcVal + sqrt(obj.squaredError(groundtruth,mu)/length(groundtruth(:)));
                    if exist('muMonitoring','var')
                        fprintf(' %f ',obj.squaredError(groundtruthMonitoring,muMonitoring)/length(groundtruthMonitoring(:)));
                    end
                case 'llh'
                ...
                case 'abs'
                    funcVal = funcVal + obj.absError(groundtruth,mu)/length(groundtruth(:));
                    if exist('muMonitoring','var')
                        fprintf(' %f ',obj.absError(groundtruthMonitoring,muMonitoring)/length(groundtruthMonitoring(:)));
                    end
            end
...
%             obj.hyperParameterObject.updateKernelReferenceSets(obj.wholeData);
    ...
    methods
        function [mu, var] = observedOneStepPredictions(obj, states, observations)
            ...
            [mu, var] = obj.hyperParameterObject.filter.outputTransformation(m,v,1);
        ...
        function [mu, var] = oneStepPredictions(obj, states)
            ...
            [mu, var] = obj.hyperParameterObject.filter.outputTransformation(m,obj.hyperParameterObject.filter.initialCov,1);
    ...
    methods (Static)
        ...
        function error = absError(data, estimates)
            error = abs(estimates - data);
            error = sum(error(:));
        end
