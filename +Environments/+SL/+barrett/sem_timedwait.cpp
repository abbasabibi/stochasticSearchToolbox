...
//       printf(" In sem_timedwait: sem %d, Timeout val: %d\n",sem, abs_timeout->tv_nsec);
      if ((abs_timeout->tv_nsec < 0) || (abs_timeout->tv_nsec > 1000000000)) {
         
         /* Passed time is invalid */
          
//          printf(" In sem_timedwait: Time invalid\n");
         
         result = -1;
         errno = EINVAL;
         
      } else {
//         printf(" In sem_timedwait: Wating\n");
...
//          printf(" In sem_timedwait: secsToWait %d, nSecsToWait %d\n", secsToWait, nsecsToWait);
         while (nsecsToWait < 0) {
            nsecsToWait += 1000000000;
            secsToWait--;
         }
//          printf(" In sem_timedwait after shift: secsToWait %d, nSecsToWait %d\n", secsToWait, nsecsToWait);
         if ((secsToWait < 0) || ((secsToWait == 0) && (nsecsToWait < 0))) {
            
            /*  Time has passed. Report an immediate timeout. */
//             printf(" In sem_timedwait: Wating over\n");
