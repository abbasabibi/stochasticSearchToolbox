...
classdef CigarReward < Environments.EpisodicContextualParameterLearningTask
    ...
    methods
        function obj = CigarReward(episodeSampler, dimContext, dimParameters)
            ...
            obj.A = randn(dimContext,dimParameters) + 10*ones(dimContext , dimParameters);
        ...
        function [reward] = sampleReturn(obj, contexts, parameters)
           ...
           for(i=1:size(parameters,1))
            ...
            reward = x(:,1).^2 + 1e6*sum(x(:,2:end).^2,2);% + 1000 * contexts ;
