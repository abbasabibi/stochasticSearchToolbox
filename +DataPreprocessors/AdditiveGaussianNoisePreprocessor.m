classdef AdditiveGaussianNoisePreprocessor < DataPreprocessors.DataPreprocessor & Data.DataManipulator
    ...
    methods
        function obj = AdditiveGaussianNoisePreprocessor(dataManager, preprocessorName)
            ...
            obj.unlinkProperty([obj.name '_sigma']);
            obj.linkProperty('positiveOnly',[obj.name '_positiveOnly']);
        ...
        function varargout = getNoisyVariates(obj, varargin)
            ...
            for i = 1:length(varargin)
                noise = randn(size(varargin{i})) * chol(obj.sigma);
                if obj.positiveOnly
                    noise(noise < 0) = 0;
                end
                varargout{i} = varargin{i} + noise;
            end
        end
